...
### 💾 Installing `{{ repo_name }}` in Development mode
...
Project scaffold generated by Copier.

To update template-derived files later:

```sh
copier update
```

{% if add_example_module %}
Example:

```python
from {% if package_namespace %}{{ package_namespace }}.{% endif %}{{ package_name }}.hello import say_hi
print(say_hi("world"))
```
{% endif %}
